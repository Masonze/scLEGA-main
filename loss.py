...
class ZINBLoss(torch.nn.Module):
    ...
    def forward(self, mean, pi, tailor_rate,loss_rate,target, theta=None,):
        cols = target.shape[1]
        start = int(cols * tailor_rate)
        end = cols
        slice = target[:, start:end]
        slice = slice * loss_rate
        target[:, start:end] = slice
...
def train_em(model_dict, loss_dict, model, loss,
    ...
    for i in it:
        ...
        if verbose:
            text = 'Epoch: %s training loss: %s val loss: %s' % (
            (i + 1), ret['loss'][-1], ret['val_loss'][-1] if 'val_loss' in ret else '---')
